...
public class OSMSaxParser extends DefaultHandler {
     ...
     */
    public Map<String, Element> parse(InputStream f) throws IOException, SAXException {
        return parse(new InputSource(f));
    }
     ...
     *             If an error occurs during parsing
     */
    public Map<String, Element> parse(String s) throws SAXException,
            IOException {
        return parse(new InputSource(new ByteArrayInputStream(
                s.getBytes("UTF-8"))));
    }
     ...
     *             If an error occurs during parsing
     */
    public Map<String, Element> parse(InputSource input) throws SAXException,
            IOException {
        // Init elements set
    ...
    public void endElement(String uri, String localName, String qName)
        ...
        if (localName.equals("node") || localName.equals("way")
                || localName.equals("relation")) {
            // Add element to list, and delete current
            if (current != null) {
                if ((localName.equals("way") && ((Way) current).getNodes()
                        .size() >= 2)
                        || localName.equals("node")
                        || (localName.equals("relation") && ((Relation) current)
                        .getMembers().size() > 0)) {
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        if (localName.equals("node")) {
            NodeT n = new NodeT(Long.parseLong(attributes.getValue("id")),
        ...
        // Case of way node
            ...
            ((Way) current).addNode((NodeT) elements.get("N"
        ...
        // Case of relation member
            ...
            String refMember = getId(attributes.getValue("type"),
                    attributes.getValue("ref"));
            ...
            if (!elements.containsKey(refMember)) {
                switch (attributes.getValue("type")) {
                    case "node":
                        elemMember = new NodeT(Long.parseLong(attributes
                                .getValue("ref")), 0, 0);
                        break;
                    case "way":
                        elemMember = new Way(Long.parseLong(attributes
                                .getValue("ref")));
                        break;
                    case "relation":
                        elemMember = new Relation(Long.parseLong(attributes
                                .getValue("ref")));
                        break;
                }
            } else {
            ...
            // Add member
            ((Relation) current).addMember(attributes.getValue("role"),
                    elemMember);
        }
        // Case of tag
        else if (localName.equals("tag")) {
            if (current != null) {
                current.addTag(attributes.getValue("k"),
